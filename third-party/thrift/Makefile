...
default: $(THRIFT_DIRECTORY)
	...
	cd $< && ./configure --prefix=$(INSTALL_PREFIX) --exec-prefix=$(INSTALL_PREFIX) --dataroot=$(CURDIR)/data/ --with-boost=$(INSTALL_PREFIX)/ --enable-tutorial=no --enable-tests=no --with-erlang=no --with-java=yes --with-php=no --with-python=no --with-cpp=yes --with-perl=yes --enable-tests=no && make && make install
