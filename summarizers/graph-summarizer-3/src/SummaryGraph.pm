...
=cut
    ...
    # 5 - enrich graph with node/edge features
    print STDERR "\tEnriching nodes/edges ...\n";
    foreach my $node ( $gist_graph->vertices() ) {
	
	# ********************************************************************************************************************************** #
	# node features
	my $node_prior = $gist_graph->get_vertex_weight( $node ) / ( $gist_graph->get_graph_attribute( $Web::Summarizer::Graph2::Definitions::GRAPH_PROPERTY_PATH_COUNT ) || 1 );
	# TODO: my $node_prior_content = ...
	$gist_graph->set_vertex_attribute( $node , $Web::Summarizer::Graph2::Definitions::NODE_ATTRIBUTE_PRIOR , $node_prior );
	# ********************************************************************************************************************************** #

	# ********************************************************************************************************************************** #
	# process all outgoing edges
	foreach my $successor ( $gist_graph->successors( $node ) ) {
	    my $edge_prior = $gist_graph->get_edge_attribute( $node , $successor , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_WIDTH ) / ( $gist_graph->get_graph_attribute( $Web::Summarizer::Graph2::Definitions::GRAPH_PROPERTY_PATH_COUNT ) || 1 );
	    $gist_graph->set_edge_attribute( $node , $successor , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_PRIOR , $edge_prior );
	    $gist_graph->set_edge_attribute( $node , $successor , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_FEATURES , encode_json( _edge_feature_ids( [ $node , $successor ] ) ) );
	}
	# ********************************************************************************************************************************** #
    ...
    my $output_file = join("/", $output_directory, "graph.raw");
    my $output_file_dot = join("/", $output_directory, "graph.raw.dot");
    my $writer_dot = Graph::Writer::Dot->new();
    my $writer = Graph::Writer::XML->new();
    $writer->write_graph($gist_graph, $output_file);
    $writer_dot->write_graph($gist_graph, $output_file_dot);
...
sub _generate_feature {
	...
	#my $edge_prior = $graph->get_edge_attribute( @{ $graph_edge } , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_PRIOR );
	#my $edge_frequency_in_target = $graph->get_edge_attribute( @{ $graph_edge } , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_TARGET_FREQUENCY );
	#my $edge_frequency_expected = $graph->get_edge_attribute( @{ $graph_edge } , $Web::Summarizer::Graph2::Definitions::EDGE_ATTRIBUTE_EXPECTED_FREQUENCY );
	#$feature_value = _adjusted_feature_value( $edge_prior , $edge_frequency_in_target , $edge_frequency_expected );
